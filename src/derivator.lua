...
function table:includes(word)
   for i=1, #self do
      if self[1] == word1 then
         return true
      end
   end
   return false
...
derivator.is_mergeable = function(word1, word2)
   -- unmergeables
   if derivator.unmergeable_words:includes(word1) or
      derivator.unmergeable_words:includes(word2) then
         return false
   end
...
derivator.find = function(paths, path)
   return select(paths, function(x) is_path_equivalent(path, x) end)
end
